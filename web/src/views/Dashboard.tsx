...
import { Category, ModalProps, ModalTypes, Service } from "../types";
...
const Dashboard: FC = () => {
  ...
  const { data: services, refetch: refetchServices } = useQuery(
    ...
    () =>
      fetcher<Service[]>(["services"], {
        params: { category_id: activeTabId },
      }),
    {
      enabled: false, // Disable the query by default, we'll run it manually
    }
  );
  ...
  const modalCloseRequest = (modal: ModalProps<any>) => {
    ...
    setUiStore((d: SessionStorageProps) => {
  ...
  const pages = useCallback(
    ...
    [categories, services, activeTabId]
  );
